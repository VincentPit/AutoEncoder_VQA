...
class MultiModalModel(nn.Module):
    def __init__(self, bert_model, vit_model, tokenizer, vocab_size, max_seq_length=1024):
        ...
        self.positional_embedding = PositionalEmbedding(max_seq_length, 1024)
    ...
    def forward(self, text_input_ids, text_attention_mask, image_tensor, decoder_input_ids):
        ...
        # Pad BERT embeddings to match ViT size (1024)
        pad_size = 1024 - text_embeddings.size(-1)
        padding = torch.zeros(text_embeddings.size(0), text_embeddings.size(1), pad_size).to(text_embeddings.device)
        text_embeddings = torch.cat([text_embeddings, padding], dim=-1)  # Shape: [batch_size, seq_len, 1024]
