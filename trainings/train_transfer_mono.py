...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        # Evaluate on a subset of evaluation data
        eval_indices = random.sample(range(len(eval_dataloader.dataset)), 5)
        for idx in eval_indices:
            eval_example = eval_dataloader.dataset[idx]
            image_path = os.path.join(img_dir, f'COCO_train2014_{eval_example["img_id"]:012d}.jpg')
    ...
    # Save the model
    torch.save(model.module.state_dict(), 'transfer_cross_attention_upgraded.pth')
